defmodule Heatwave.TemperatureControllerTest do
  ...
  describe "temperatures" do
    ...
    test "create_temperature/1 with valid data creates a temperature" do
      ...
      assert {:ok, %Temperature{} = temperature} = TemperatureController.create_temperature(valid_attrs)
    ...
    test "create_temperature/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = TemperatureController.create_temperature(@invalid_attrs)
    end
    ...
    test "update_temperature/2 with valid data updates the temperature" do
      ...
      assert {:ok, %Temperature{} = temperature} = TemperatureController.update_temperature(temperature, update_attrs)
    ...
    test "update_temperature/2 with invalid data returns error changeset" do
      temperature = temperature_fixture()
      assert {:error, %Ecto.Changeset{}} = TemperatureController.update_temperature(temperature, @invalid_attrs)
    ...
    test "delete_temperature/1 deletes the temperature" do
      ...
      assert_raise Ecto.NoResultsError, fn -> TemperatureController.get_temperature!(temperature.id) end
    end
